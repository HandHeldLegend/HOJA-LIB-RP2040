...
void triggers_start_calibration()
{
    ...
    global_loaded_settings.trigger_l.lower = 0x7FF;
    global_loaded_settings.trigger_r.lower = 0x7FF;
...
void triggers_scale(int left_trigger_in, int *left_trigger_out, int right_trigger_in, int *right_trigger_out)
{
    ...
    if(_trigger_calibration)
    {
        ...
        if( (left_trigger_in+TRIGGER_DEADZONE)  < (int)global_loaded_settings.trigger_l.lower) global_loaded_settings.trigger_l.lower = (uint16_t) left_trigger_in+TRIGGER_DEADZONE;
        if( (right_trigger_in+TRIGGER_DEADZONE) < (int)global_loaded_settings.trigger_r.lower) global_loaded_settings.trigger_r.lower = (uint16_t) right_trigger_in+TRIGGER_DEADZONE;
...
void triggers_scale_init()
{
    ...
    if(global_loaded_settings.trigger_l.upper>0)
    {
        ...
        range = global_loaded_settings.trigger_l.upper - (global_loaded_settings.trigger_l.lower);
    ...
    {
        // First calculate our full range
        range = global_loaded_settings.trigger_r.upper - (global_loaded_settings.trigger_r.lower);
