...
uint32_t _haptics_get_lookup_index(float input)
{
...
float _haptics_apply_command(Switch5BitAction_t action, float offset, float current, float default_val, float min, float max)
{
    switch (action)
    {
...
void _haptics_initialize_lookup_tables(void)
{
...
bool _haptics_disabled_check(uint8_t *data)
{
...
void _haptics_linear_set_default(hoja_haptic_frame_linear_s *state)
{
...
void _haptics_linear_to_normal(hoja_haptic_frame_linear_s *linear, hoja_haptic_frame_s *decoded)
{
    decoded->high_frequency = ExpBase2Lookup[_haptics_get_lookup_index(linear->hi_freq_linear)] * CenterFreqHigh;
    decoded->low_frequency = ExpBase2Lookup[_haptics_get_lookup_index(linear->lo_freq_linear)] * CenterFreqLow;
    decoded->high_amplitude = ExpBase2Lookup[_haptics_get_lookup_index(linear->hi_amp_linear)];
    decoded->low_amplitude = ExpBase2Lookup[_haptics_get_lookup_index(linear->lo_amp_linear)];
}
...
void _haptics_decode_type_1(const SwitchHapticPacket_s *encoded, hoja_rumble_msg_s *decoded)
{
    ...
    if (samples > 0)
    {
        ...
        decoded->linear.hi_freq_linear = _haptics_apply_command(hi_cmd.fm_action, hi_cmd.fm_offset,
                                                               decoded->linear.hi_freq_linear, DefaultFrequency, MinFrequency, MaxFrequency);
        decoded->linear.hi_amp_linear = _haptics_apply_command(hi_cmd.am_action, hi_cmd.am_offset,
                                                              decoded->linear.hi_amp_linear, DefaultAmplitude, MinAmplitude, MaxAmplitude);

        low_cmd = CommandTable[encoded->type1.cmd_lo_0];
        decoded->linear.lo_freq_linear = _haptics_apply_command(low_cmd.fm_action, low_cmd.fm_offset,
                                                               decoded->linear.lo_freq_linear, DefaultFrequency, MinFrequency, MaxFrequency);
        decoded->linear.lo_amp_linear = _haptics_apply_command(low_cmd.am_action, low_cmd.am_offset,
                                                              decoded->linear.lo_amp_linear, DefaultAmplitude, MinAmplitude, MaxAmplitude);

        _haptics_linear_to_normal(&(decoded->linear), &(decoded->samples[0]));
    }
    ...
    if (samples > 1)
    {
        ...
        _haptics_linear_to_normal(&(decoded->linear), &(decoded->samples[1]));
    }
    ...
    if (samples > 2)
    {
        ...
        _haptics_linear_to_normal(&(decoded->linear), &(decoded->samples[2]));
    }
}
...
{
    hoja_rumble_msg_s internal_left = {0};
    hoja_rumble_msg_s internal_right = {0};
    ...
    if (!_haptics_init)
    {
        ...
        _haptics_initialize_lookup_tables();
        _haptics_init = true;
    }
    ...
    for(int i = 0; i < internal_left.sample_count; i++)
    {
    ...
    pcm_try_push_amfm(l_set, internal_left.sample_count);
}
